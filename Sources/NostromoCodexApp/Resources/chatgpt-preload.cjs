...
const SUPPORTED_VERSIONS = new Set(["26.721.41059", "26.721.81911"]);
...
class VirtualHIDAsyncDevice extends EventEmitter {
  ...
  attachSocket(socket, initialBuffer = "") {
    ...
    this.writeLine(runtimeCapabilityManifest()).catch((error) => {
      if (this.isConnectionActive(socket, generation)) this.emitAsyncError(error);
    });
